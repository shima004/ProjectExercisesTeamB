...
func NewField(timeLimit int64, FPS int64, size Point2D, paddleSize Point2D, paddleVelocity Point2D, ballSize float64, ballVelocity Point2D, ballMaxVelocity float64, initVelocity Point2D) Field {
	return Field{
		Time:       0,
		TimeLimit:  timeLimit,
		FPS:        FPS,
		Ball:       NewBall(NewPoint2D(size.X/2, size.Y/2), ballVelocity, initVelocity, ballSize, ballMaxVelocity),
